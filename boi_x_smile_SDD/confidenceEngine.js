...
// คำนวณ weighted confidence score จาก vector similarity + LLM self-assessment
...
/**
 * คำนวณ weighted_final score
 * @param {number} vectorSimilarity - score จาก Vector DB retrieval (0–1)
 * @param {number} llmSelfScore     - score ที่ Claude ประเมินตัวเอง (0–1)
 * @returns {{ vectorSimilarity, llmSelfScore, weightedFinal, level }}
 */
function calculateConfidence(vectorSimilarity, llmSelfScore) {
  ...
  const weightedFinal =
    vectorSimilarity * WEIGHT_VECTOR + llmSelfScore * WEIGHT_LLM;
...
/**
 * ตัดสินใจ action จาก confidence level
 * @returns {'tag' | 'tag_with_flag' | 'refuse'}
 */
...
/**
 * ตรวจว่าควร escalate ไป human หรือไม่
 */
